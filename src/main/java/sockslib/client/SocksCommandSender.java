...
public interface SocksCommandSender {
  ...
  int REP_GENERAL_SOCKS_SERVER_FAILURE = 0x01;
  int REP_CONNECTION_NOT_ALLOWED_BY_RULESET = 0x02;
  int REP_NETWORK_UNREACHABLE = 0x03;
  int REP_HOST_UNREACHABLE = 0x04;
  int REP_CONNECTION_REFUSED = 0x05;
  int REP_TTL_EXPIRED = 0x06;
  int REP_COMMAND_NOT_SUPPORTED = 0x07;
  int REP_ADDRESS_TYPE_NOT_SUPPORTED = 0x08;
   ...
   */
  CommandReplyMessage send(Socket socket, SocksCommand command, InetAddress address, int
   ...
   * @throws IOException If any I/O error occurs.
   */
  CommandReplyMessage send(Socket socket, SocksCommand command, SocketAddress address, int
   ...
   * @throws IOException If any I/O error occurs.
   */
  CommandReplyMessage send(Socket socket, SocksCommand command, String host, int port, int
      version) throws SocksException, IOException;

  CommandReplyMessage checkServerReply(InputStream inputStream) throws
      IOException;
}
